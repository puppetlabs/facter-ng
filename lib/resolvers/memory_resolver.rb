    ...
    module Linux
      class Memory < BaseResolver
        ...
        class << self
          ...
          def read_meminfo_file(_fact_name)
          ...
          @fact_list[:used_bytes] = (@fact_list[:total] - @fact_list[:memfree]) * 1024
          @fact_list[:sused_bytes] = (@fact_list[:swap_total] - @fact_list[:swap_free]) * 1024
          @fact_list[:capacity] = ((@fact_list[:used_bytes] / @fact_list[:total]) * (100 / 100)) * 1024
          @fact_list[:scapacity] = ((@fact_list[:sused_bytes] / @fact_list[:swap_total]) * (100 / 100)) * 1024
          @fact_list[fact_name]
          end
      end
    end
