...
describe 'Windows HardwareArchitectureResolver' do
  ...
  after do
    HardwareArchitectureResolver.invalidate_cache
  end
  ...
  context '#resolve when processor is amd64' do
    ...
    it 'detects hardware' do
      expect(HardwareArchitectureResolver.resolve(:hardware)).to eql('x86_64')
    end
    it 'detects architecture' do
      expect(HardwareArchitectureResolver.resolve(:architecture)).to eql('x64')
    end
  end
  ...
  context '#resolve when processor is arm' do
    ...
    it 'detects hardware' do
      expect(HardwareArchitectureResolver.resolve(:hardware)).to eql('arm')
    end
    it 'detects architecture' do
      expect(HardwareArchitectureResolver.resolve(:architecture)).to eql('arm')
    end
  end
  ...
  context '#resolve when processor is ia64' do
    ...
    it 'detects hardware' do
      expect(HardwareArchitectureResolver.resolve(:hardware)).to eql('ia64')
    end
    it 'detects architecture' do
      expect(HardwareArchitectureResolver.resolve(:architecture)).to eql('ia64')
    end
  end
  ...
  context '#resolve when processor is intel and level below 5' do
    ...
    it 'detects hardware' do
      expect(HardwareArchitectureResolver.resolve(:hardware)).to eql("i#{level}86")
    end
    it 'detects architecture' do
      expect(HardwareArchitectureResolver.resolve(:architecture)).to eql('x86')
    end
  end
  ...
  context '#resolve when processor is intel and level above 5' do
    ...
    it 'detects hardware' do
      expect(HardwareArchitectureResolver.resolve(:hardware)).to eql('i686')
    end
    it 'detects architecture' do
      expect(HardwareArchitectureResolver.resolve(:architecture)).to eql('x86')
    end
  ...
  context '#resolve when processor unknown' do
    ...
    it 'detects hardware' do
      expect(HardwareArchitectureResolver.resolve(:hardware)).to eql('unknown')
    end
    it 'detects architecture' do
      expect(HardwareArchitectureResolver.resolve(:architecture)).to eql('unknown')
    end
  end
end
