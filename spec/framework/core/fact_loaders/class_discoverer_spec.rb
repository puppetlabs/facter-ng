...
describe 'ClassDiscoverer' do
  describe '#discover_classes' do
    it 'loads all classes' do
      allow_any_instance_of(Module).to receive(:constants).and_return([:NetworkInterface, :OsName])

      expect(Facter::ClassDiscoverer.instance.discover_classes('Ubuntu')).to eq([:NetworkInterface, :OsName])
    end
