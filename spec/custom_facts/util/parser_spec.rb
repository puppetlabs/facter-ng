...
describe Facter::Util::Parser do
  ...
  describe "extension_matches? function" do
    it "should match extensions when subclass uses match_extension" do
      expect(Facter::Util::Parser.extension_matches?("myfile.foobar", "foobar")).to be true
    end

    it "should match extensions when subclass uses match_extension with an array" do
      expect(Facter::Util::Parser.extension_matches?("myfile.ext1", ["ext1","ext2","ext3"])).to be true
      expect(Facter::Util::Parser.extension_matches?("myfile.ext2", ["ext1","ext2","ext3"])).to be true
      expect(Facter::Util::Parser.extension_matches?("myfile.ext3", ["ext1","ext2","ext3"])).to be true
    end

    it "should match extension ignoring case on file" do
      expect(Facter::Util::Parser.extension_matches?("myfile.EXT1", "ext1")).to be true
      expect(Facter::Util::Parser.extension_matches?("myfile.ExT1", "ext1")).to be true
      expect(Facter::Util::Parser.extension_matches?("myfile.exT1", "ext1")).to be true
    end
  ...
  describe "nothing parser" do
    it "uses the nothing parser when there is no other parser" do
      expect(Facter::Util::Parser.parser_for("this.is.not.valid").results).to be nil
    end
  end
end
